# -*- coding: utf-8 -*-
"""
Created on Mon Mar  9 21:30:15 2020
...
#          conv2 = Conv2D(self.conv2_filters, self.conv2_kernel_shape, padding = self.padding2, activation=self.conv1_activation,name = str(i+1)+"conv"+str(2))(conv1)
          conv2 = self.tfaugmented_conv2d(conv1, self.conv2_filters, self.conv2_kernel_shape, dk=self.depth_k, dv=self.depth_v, Nh=self.num_heads, relative=self.relative)
...
#          conv3 = Conv2D(self.conv3_filters, self.conv3_kernel_shape, padding = self.padding3, activation=self.conv1_activation,name = str(i+1)+"conv"+str(3))(conv2)
          conv3 = self.tfaugmented_conv2d(conv2, self.conv3_filters, self.conv3_kernel_shape, dk=self.depth_k, dv=self.depth_v, Nh=self.num_heads, relative=self.relative)
...
#    plot_model(cascade_model, show_shapes=True, to_file='cascade_self_attention.png')
