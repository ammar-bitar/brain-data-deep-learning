...
parallel_object = Parallel(window_size,conv1_filters,conv2_filters,conv3_filters,
...
def hybrid_training(setup):
    ...
    experiment_number = on_train_begin(parallel_object)
    for epoch in range(num_epochs):
        ...
        on_epoch_end(epoch, average_accuracy_epoch_train, average_loss_epoch_train, \
                        average_accuracy_epoch_validate, average_loss_epoch_validate, experiment_number, model)
...
#        if (epoch+1) % 2 == 0 :
    ...
    model_save(experiment_number,model)
